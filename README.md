...
### 2026-06-01 UI 피드백 반영

- 기본 위젯 위치를 왼쪽으로 옮기고 시계는 오른쪽으로 보냈습니다. 책상/캐릭터가 오른쪽에서 더 잘 보이길 원하는 사용자를 위해 설정에 `위젯 위치` 좌우 반전 옵션도 추가했습니다.
- `Cal`, `PiP`, `MV`, `Hide`가 하단에 텍스트로 모여 있어 의미를 파악하기 어렵다는 피드백을 반영했습니다. 캘린더와 PiP는 작은 위젯으로 승격하고, Hide는 WebSocket 상태와 종료 버튼 사이의 시각 기능 버튼으로 옮겼습니다. MV는 음악 부가기능으로 보고 오디오 설정 안으로 내렸습니다.
- 자세 측정 외 위젯은 기존보다 작게 줄였습니다. 핵심 기능인 자세 측정만 큰 위젯으로 유지하고, Focus/Notes/Tasks/Calendar/PiP/Settings는 보조 기능으로 보이도록 밀도를 낮췄습니다.
- 리포트 위젯은 제거하고 설정 안의 `오늘 리포트` 버튼으로 이동했습니다. 리포트 조회는 기존 백엔드 API를 그대로 사용하며, 로그아웃 시 리포트 호출 흐름도 유지됩니다.
- Focus 위젯을 누르면 포모도로 설정 탭이 열리도록 바꿨습니다. 작업/휴식/반복 값을 직접 수정할 수 있고, 위젯과 설정 탭 모두에서 남은 시간과 예상 종료 시간을 확인할 수 있습니다.
- 포모도로 SE 켜기/끄기 버튼이 실제로 동작하도록 연결했습니다. 끄면 페이즈 종료 알림음과 완료 알림음이 재생되지 않습니다.
- 플레이어 재생 상태가 헷갈리지 않도록 재생 중에는 `||`, 정지/대기 중에는 `|>`로 버튼 텍스트를 동기화했습니다. 메인 플레이어, phone 위젯, PiP 플레이어가 같은 상태를 표시합니다.
- 포모도로 설정에 리셋 버튼을 추가했습니다. 현재 작업/휴식 페이즈와 진행 시간을 초기 작업 시간으로 되돌립니다.
- 남는 위젯 칸에는 화이트 노이즈 위젯을 추가했습니다. Web Audio API로 2초짜리 노이즈 버퍼를 한 번 생성한 뒤 루프 재생하므로, 실시간 3D/자세 측정 성능에 부담을 주지 않도록 설계했습니다.
