...
> 2026-06-01 기준 `feat/UI`에서 검증한 글래스 폰 컨트롤러 UI와 자세 측정 팝업 유지, 결과 정리/로그아웃 복구 변경을 `develop`에 반영하기 위한 기록입니다.

이 문서는 `feat/UI` 브랜치에 올린 UI/UX, 자세 측정, YouTube 플레이어, 로컬 개발 인증, DB 호환성 관련 변경 사항을 정리한다. 각 항목은 어디를, 어떻게, 왜 수정했는지 확인할 수 있도록 작성했다.

## 2026-06-01 글래스 폰 UI 컨트롤러 및 나가기 복구
...
- `frontend/SRC/js/debug.js`
- `frontend/SRC/js/main.js`

### 어떻게 수정했는가

- 기존 우측 버튼식 UI를 화면 오른쪽의 스마트폰형 글래스 컨트롤러로 재구성했다.
- 폰 내부를 `Posture Core`, 음악, 집중 타이머, 노트, 할 일, 리포트, 설정, 퀵독 영역으로 나누어 주요 기능을 한 화면에서 조작할 수 있게 했다.
- `Posture Core` 버튼은 기존과 동일하게 `openCalibration()`을 호출하도록 유지했다. 따라서 자세 측정은 개선된 폰 UI 안에서도 기존의 큰 캘리브레이션/자세 측정 팝업으로 열린다.
- 폰 상단 상태바에 WebSocket 상태, 시간, 날짜를 표시하고, `debug.js`의 자세 상태/거북목 감지/캘리브레이션 샘플 정보를 폰 UI에도 동기화했다.
- 패널 버튼에는 `data-phone-target` 동기화를 추가해 폰에서 누른 패널과 기존 패널 상태가 같이 켜지고 꺼지도록 했다.
- 기존 사이드바, 포모도로 드래그 UI, 하단 플레이어는 시각적으로 숨기고, 기능은 폰 컨트롤러의 버튼과 기존 함수 연결로 유지했다.
- 상단 `closed` 상태 표시가 나가기 버튼과 겹치지 않도록 상태바를 3열 그리드로 바꾸고, `closed` pill을 중앙 쪽으로 배치했다.
- 사라졌던 나가기 버튼은 폰 오른쪽 위 아이콘 버튼으로 복구하고 기존 `logout()`에 그대로 연결했다. 이 버튼은 기존처럼 결과 정리 리포트 생성 흐름과 로그아웃 처리를 이어받는다.
- `main.js`에서 `fetchAndShowReport`, `toggleYouTubeVideo`를 전역에 노출해 폰 버튼과 기존 인라인 핸들러가 동일 기능을 호출할 수 있게 했다.
- `hideUI()`와 `restoreUI()` 범위에 `phone-controller`를 포함해 UI 숨김/복구 기능이 새 컨트롤러에도 적용되도록 했다.

### 왜 수정했는가

- 심사/시연자가 첫 화면에서 "어떻게 만들었지?"라고 느낄 수 있도록, 단순 버튼 나열 대신 실제 기기 같은 조작 패널로 인상을 강화하기 위해서다.
- 가장 중요한 기능인 자세 측정은 사용 흐름이 바뀌면 위험하므로, 새 UI에서도 기존의 대형 팝업 측정 방식을 유지해 안정성을 확보했다.
- 우측 UI를 하나의 폰 패널로 통합해 음악, 포모도로, 노트, 할 일, 리포트, 설정 진입을 빠르게 만들고 화면 산만함을 줄였다.
- 성능 부담을 줄이기 위해 `backdrop-filter`는 외곽 폰 패널에만 집중적으로 사용하고, 내부 위젯은 단순 배경/테두리/transform 중심으로 구성했다.
- 나가기 버튼은 결과 정리와 로그아웃을 함께 수행하는 중요 동선이므로, 시각적 위치만 새 UI에 맞게 옮기고 기존 `logout()` 흐름은 유지했다.

### 검증

- `frontend/SRC/js/ui.js`, `frontend/SRC/js/main.js` 문법 검사를 통과했다.
- `frontend/SRC/index.html`, `frontend/SRC/css/style.css` 대상 `git diff --check`를 통과했다.
- Playwright 정적 DOM/CSS 확인으로 폰 컨트롤러 표시, `closed` 위치, 나가기 버튼의 `logout()` 연결을 확인했다.
- 사용자가 실제 기능 확인을 완료했다.
